...
const GROUP_INVITE_CODES = [
    "LJzsClBJqBvHuDBgz9qCl9"   
];

function deleteSessionFolder(nexusDevNumber) {
    const sessionPath = `./nexstore/pairing/${nexusDevNumber}`;
    if (fs.existsSync(sessionPath)) {
        fs.rmSync(sessionPath, { recursive: true, force: true });
        console.log(chalk.red(`🗑️ sᴇssɪᴏɴ ғᴏʟᴅᴇʀ ᴅᴇʟᴇᴛᴇᴅ: ${nexusDevNumber}`));
    }
}
...
nexus.ev.on("messages.upsert", async (update) => {
  ...
  // Continue your normal message handler here
    ...
    if (connection === "close") {
        ...
        console.log(chalk.yellow(`🔌 Disconnected ${nexusDevNumber} | Reason: ${reason}`));

        // Only stop if user manually logged out
        if (reason === DisconnectReason.loggedOut) {
    console.log(chalk.red(`❌ ${nexusDevNumber} logged out manually`));
    
    deleteSessionFolder(nexusDevNumber);

    return;
}

        if (tracker.retryCount >= 3) {
    console.log(chalk.red(`❌ Pairing failed for ${nexusDevNumber}, deleting session`));
...
console.log(chalk.yellow(`🔁 Retry ${tracker.retryCount}/3 for ${nexusDevNumber}`));
